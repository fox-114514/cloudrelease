...
program
    ...
    try {
        ...
        const device = await bindWithLogin(options.server, options.user, password, options.name, options.profile, { allowInsecureHttp: options.allowInsecureHttp === true });
        const config = await loadConfig();
        config.device = device;
        config.downloadDir = config.downloadDir || defaultDownloadDir();
        config.allowInsecureHttp = config.allowInsecureHttp || options.allowInsecureHttp === true;
...
for (const commandName of ["whoami", "permissions", "refresh-permissions"]) {
    program
        .command(commandName)
...
program
    ...
    try {
        ...
        console.log("Permission refresh failed; showing cached identity:", err.message);
    }
...
program
    ...
    const permissionTimer = setInterval(() => {
        void refreshAndSaveDevice()
            ...
            .catch((err) => console.error(`Permission refresh failed: ${err.message}`));
    }, 5 * 60 * 1000);
    installShutdown(() => {
        clearInterval(permissionTimer);
        client.stop();
